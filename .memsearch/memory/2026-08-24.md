### 00:34
<!-- session:94084d66-9f38-42fe-bd0a-adce563419e2 turn:08733fe3-5272-4419-bbc0-a0f6bd815195 transcript:C:\Users\guido\.claude\projects\D--IT-Projects-Termdroid\94084d66-9f38-42fe-bd0a-adce563419e2.jsonl -->
- User preguntó si es viable crear una aplicación para Android que permita ejecutar una terminal, un agente local en CLI, o Claude Code para realizar tareas como consultar qué apps tiene instaladas, cuáles usa más, y otras funciones similares a las disponibles en Windows o Linux, en su Samsung Galaxy A56.

- Claude Code confirmó que es viable y analizó las capacidades del A56 (Exynos 1580, arm64, 8-12 GB RAM, Android 16), identificando que el cuello de botella principal es el modelo de permisos de Android, no el hardware.

- Se explicó la primera capa: instalar Termux desde F-Droid (no Play Store) e instalar herramientas como Node.js 22, git, Python, OpenSSH, y Claude Code vía npm, con un teclado que soporte Ctrl/Esc/Tab.

- Se presentó la segunda capa: usar ADB sobre loopback (depuración inalámbrica del teléfono contra sí mismo en Android 11+) para acceder a datos protegidos como `UsageStatsManager`, apps instaladas, estadísticas de uso, batería y notificaciones sin necesidad de root.

- Se mencionó la tercera capa: Termux:API, un APK complementario que expone sensores y servicios como comandos independientes, aunque no cubre estadísticas de uso.

- Se enumeraron limitaciones: no hay automatización de apps (requiere APK propia con permisos de accesibilidad), la persistencia en background es limitada, el bootloader bloqueado impide root, y los modelos locales como llama.cpp son demasiado lentos.

- Se presentaron cuatro opciones alternativas en una tabla con niveles de esfuerzo: SSH a PC/VPS, Termux + Claude Code + ADB loopback (recomendado inicial), agregar una APK companion propia, o crear una app terminal 100% personalizada.

- Se recomendó una arquitectura donde el agente reside en Termux y los permisos en una APK fina, exponiendo capacidades nuevas como endpoints HTTP o MCP server.

- Claude Code propuso un camino específico: instalar Termux → Node + Claude Code → validar login y repos → agregar ADB loopback con wrappers que parseen dumpsys a JSON → luego considerar la APK companion si el flujo resulta útil.

- Claude Code ofreció construir el script de bootstrap de Termux o diseñar la APK companion primero, según la preferencia del User.

### 00:41
<!-- session:94084d66-9f38-42fe-bd0a-adce563419e2 turn:10f4f03d-3364-4460-8063-e0c8057cf87b transcript:C:\Users\guido\.claude\projects\D--IT-Projects-Termdroid\94084d66-9f38-42fe-bd0a-adce563419e2.jsonl -->
=== Transcript of a conversation between User and Claude Code ===
[User]: Prefiero una propia porque no pude bajarme shiraku y termux con comodidad, la primera me dijo que no estaba disenado para androids modernos y demas, entonces me gustaria simplemente hacer la mejor app posible para eso. Priorizando perfecta ux ui y a la vez logrando que la IA tenga via libre como si se estuviese ejecutando en uan pc
[User]: Base directory for this skill: C:\Users\guido\AppData\Local\Temp\claude\bundled-skills\2.1.241\6a03a4356c5134007dc7dc1f9393f4b0\claude-api
...
Kotlin/Android agent loop with tool use against Messages API
[Claude Code]: Leí el proyecto (`D:\IT\Projects\Termdroid` está vacío — greenfield) y la referencia del SDK Java/Kotlin. Acá va el diseño.

---

# Termdroid — arquitectura

## 1. El problema central que hunde a todas las apps existentes

Lo que te pasó con Shizuku y Termux tiene una sola causa técnica: **W^X**. Desde Android 10, SELinux le niega al dominio `untrusted_app` el permiso `execute_no_trans` sobre `app_data_file`. Traducción: una app con `targetSdk ≥ 29` **no puede `execve()` un binario que escribió en su propio directorio**.

Por eso Termux está congelado en `targetSdk 28` — y por eso Android te avisa que "no está diseñada para versiones modernas", pierde acceso a scoped storage, notificaciones modernas, y cada release de Android amenaza con romperla del todo. No es descuido de los mantenedores; es una pared.

Hay tres salidas. La app se define por cuál elegís:

| Salida | Cómo | Costo |
|---|---|---|
| **A. Binarios como `.so`** | Todo ejecutable se empaqueta en `jniLibs/arm64-v8a/lib*.so`. `nativeLibraryDir` sí es ejecutable en cualquier targetSdk. | Funciona 100% seguro, pero **el set de binarios es fijo al build**. No podés `apt install`. |
| **B. Linker explícito** | `execve("/system/bin/linker64", ["linker64", "/ruta/al/binario", ...args])`. El `execve` real es sobre `linker64` (`system_file`, permitido); el binario del rootfs solo se *lee* y se mapea. | Habilita binarios instalados en runtime. Requiere ELF dinámico PIE, y manejar shebangs a mano. **Hay que verificarlo empíricamente en tu A56 / Android 16** — es la apuesta técnica del proyecto. |
| **C. targetSdk 28** | Lo de Termux. | Descartado: es exactamente el problema que querés evitar. |

**Mi recomendación: A como base garantizada, B como capa de extensión.** El rootfs mínimo (busybox/toybox, bash, coreutils, git, python, ripgrep) viaja como `.so` y funciona siempre. Encima, un *exec shim* que intercepta cualquier ejecución fuera de `nativeLibraryDir` y la rutea por el linker. Si B falla en algún device, la app sigue siendo usable en vez de morir.

> Escape hatch a futuro: Android 15+ trae **Linux Terminal** sobre AVF (Debian en pKVM), que resuelve todo esto de raíz. Hoy solo en Pixel; Samsung no expone pKVM. Vale diseñar la capa de ejecución detrás de una interfaz para poder enchufarlo cuando llegue.

## 2. El agente corre en Kotlin, no en Node

Segunda decisión fuerte, y creo que la más importante para tu objetivo de UX.

Meter Node en el device para correr Claude Code te da un agente diseñado para una TTY de escritorio, con una UX que en un teléfono es hostil, y te ata al problema de la sección anterior en su forma más difícil (npm instala binarios nativos).

En cambio: **implementar el agent loop directamente en Kotlin contra la Messages API**, con el SDK oficial (`com.anthropic:anthropic-java`, el mismo que usa Kotlin). El loop es ~200 líneas: mandás `tools`, recibís `stop_reason: "tool_use"`, ejecutás, devolvés `tool_result`, repetís.

Lo que ganás:
- La UI conoce cada tool call **antes** de ejecutarlo → aprobación inline, diffs renderizados, cancelación real.
- Streaming nativo hacia Compose, incluido el thinking (`display: "summarized"`).
- Cero runtime frágil. La app es un APK y ya.
- El "modo terminal" pasa a ser *una tool más* (`bash`), no el fundamento de todo.

Config base: `claude-opus-5`, `thinking: {type: "adaptive"}`, `output_config.effort: "xhigh"` (el sweet spot para trabajo agéntico), streaming siempre, `max_tokens` alto, y compaction server-side (`compact-2026-01-12`) para que las sesiones largas no se caigan.

Sobre credenciales: la API key en el device es el punto débil. Guardala en el **Android Keystore** con `setUserAuthenticationRequired`, y dejá la puerta abierta a un proxy propio o OAuth más adelante.

## 3. Las tools: qué significa "vía libre"

Acá está lo que ninguna terminal te puede dar y una app propia sí, porque los permisos se piden como app.

**Capa Unix** (contra el rootfs): `bash`, `read_file`, `write_file`, `edit_file`, `glob`, `grep`.

**Capa Android** (lo que preguntaste — "qué apps tengo, cuál uso más"):

| Tool | API | Permiso |
|---|---|---|
| `list_apps` | `PackageManager` | `QUERY_ALL_PACKAGES` |
| `app_usage` | `UsageStatsManager` | `PACKAGE_USAGE_STATS` (Settings intent, una vez) |
| `notifications` | `NotificationListenerService` | toggle en Settings |
| `device_state` | batería, red, storage, sensores | normales |
| `media` / `files` | MediaStore, SAF | scoped storage |
| `ui_action` | `AccessibilityService` | toggle en Settings |
| `shell_priv` | **ADB embebido** (abajo) | pairing una vez |

**El canal privilegiado, sin Shizuku:** en vez de depender de una app externa que se rompe, embebés el cliente ADB en Kotlin (`libadb-android`: pairing TLS + protocolo ADB sobre `127.0.0.1`). El usuario activa Depuración inalámbrica, pega el código de pairing **una sola vez**, y la app levanta un servicio propio con UID `shell` (2000). Desde ahí: `dumpsys`, `pm`, `cmd`, `settings`. Es el mismo mecanismo de Shizuku pero integrado, con onboarding que vos controlás — que es precisamente donde Shizuku falla.

## 4. Procesos

```
:ui       Compose · chat + terminal + aprobaciones
:agent    Foreground Service · loop, streaming, sesiones (persistente ante rotación y background)
:shell    isolated · rootfs, PTY nativo (JNI), un proceso por sesión
:adb      cliente ADB embebido → UID shell
```

El `:agent` como Foreground Service con `dataSync` es no-negociable: One UI mata background con saña, y necesitás que una tarea larga sobreviva a que apagues la pantalla.

## 5. UX/UI — el diferenciador

Esta es la parte donde "no importa que no exista previamente" tiene más sentido, porque **no existe**. Todas las terminales Android son un `xterm` empujado a una pantalla de 6".

Principios:

**Chat primero, terminal segundo.** La pantalla principal es la conversación con el agente. El terminal vive en un bottom sheet que se expande cuando querés manejar el shell vos. Invertí la jerarquía respecto de Termux: el 90% del tiempo no querés tipear comandos, querés pedir cosas.

**Los tool calls son componentes, no texto.** Un `edit_file` se renderiza como una card con diff coloreado y botones Aprobar/Rechazar. Un `bash` muestra el comando en mono con output colapsable. Un `app_usage` devuelve **un gráfico**, no una tabla ASCII. El agente devuelve datos estructurados; la app decide cómo se ven.

**La barra de teclado es contextual.** Nada de "Hacker's Keyboard". Cuando el foco está en el terminal: `Esc Tab Ctrl / - | ~ ↑↓`. Cuando está en el chat: `@archivo`, `/comando`, adjuntar, dictar. Cambia sola.

**Modos de autonomía explícitos**, con un selector siempre visible: *Preguntar todo* → *Auto-lectura* → *Auto-todo*. El usuario nunca se pregunta qué está por hacer el agente.

**El teléfono es el contexto.** Compartir desde cualquier app → llega al agente. Quick Settings tile para "preguntá algo". Widget con el estado de la tarea corriendo. Notificación viva con progreso y botón de cancelar. Esto es lo que hace que valga la pena que corra en el teléfono y no por SSH.

**Sesiones persistentes.** Cerrás la app, volvés, la conversación y el proceso siguen. Room + el service.

## 6. Fases

1. **Prueba de fuego** — un spike de 1-2 días que valide (B) en tu A56: rootfs mínimo como `.so`, PTY por JNI, y el truco del linker. **Si (B) no funciona, el alcance cambia y conviene saberlo ahora.**
2. **Terminal decente** — emulador VT + Compose + barra contextual. Comparable a Termux, sin la deuda de targetSdk.
3. **Agente** — loop Kotlin, tools Unix, streaming, aprobaciones, sesiones.
4. **Vía libre** — permisos Android + tools de sistema + ADB embebido.
5. **Integración al teléfono** — share target, tile, widget, voz.

## 7. Riesgos honestos

- **(B) puede no funcionar** en Android 16 / One UI 8. Es el riesgo #1 y por eso va en la fase 1.
- **Play Store**: `QUERY_ALL_PACKAGES` + `PACKAGE_USAGE_STATS` + Accessibility en una sola app es rechazo casi seguro. Distribución por APK/GitHub. Si querés Play, hay que partir la app.
- **Emulador de terminal**: escribir un VT100/xterm completo es meses. `terminal-emulator` de Termux es Apache 2.0 — reusalo, es la parte menos diferenciadora.
- **Batería**: sesión de agente = red + CPU sostenidos. Necesitás límites explícitos y visibles.

---

**Próximo paso:** armo el spike de la fase 1 (proyecto Gradle, rootfs mínimo empaquetado como `.so`, PTY por JNI, y el test del linker explícito) para que en un par de horas sepamos si el camino aguanta. ¿Arranco por ahí, o preferís que primero escriba la propuesta OpenSpec completa con el diseño cerrado?


## Session 13:43

### 13:53
<!-- session:94084d66-9f38-42fe-bd0a-adce563419e2 turn:a894e122-858f-4b65-a352-013c197f91af transcript:C:\Users\guido\.claude\projects\D--IT-Projects-Termdroid\94084d66-9f38-42fe-bd0a-adce563419e2.jsonl -->
- User establece un goal con hook que bloquea hasta completar la planificación perfecta del proyecto Termdroid.
- Claude Code identifica y cierra tres huecos técnicos del plan anterior que afectaban decisiones de diseño fundamental.
- Claude Code determina que los binarios deben compilarse como bionic (compatible con linker64 de Android) en lugar de musl o glibc, descartando Alpine/Debian como base.
- Claude Code propone un fork del build system de termux-packages con TERMUX_PREFIX propio como solución coherente de toolchain, evitando el prefix hardcodeado de Termux.
- Claude Code escribe y cierra el plan completo en la vault bajo `20_PROJECTS/PERSONAL/Termdroid/` con 23 archivos, ninguno vacío, incluyendo documentos de decisiones, conocimiento y seis ADRs.
- Claude Code documenta tres cambios principales en el diseño: (1) toolchain determinado por la capa de ejecución, (2) exec shim con LD_PRELOAD absorbiendo dos niveles, (3) modelo de seguridad como documento propio con regla central sobre datos vs instrucciones.
- Claude Code especifica que F-001 es un gate real con tres resultados posibles (ambos niveles andan, solo nivel 1, falla nivel 1) que determina si continúa F-002.
- Claude Code marca dos items a verificar antes de continuar: licencia real del emulador terminal y comportamiento de One UI matando Foreground Service en A56.
- Claude Code especifica configuración del agente: claude-opus-5 con adaptive thinking summarized, effort xhigh, streaming, compaction, fallbacks server-side, usando loop manual en lugar de BetaToolRunner.
- Claude Code define el próximo paso: ejecutar C-001 bootstrap Gradle en `D:\IT\Projects\Termdroid` con módulos vacíos y spike listo para ejecutar F-001 contra el A56.
...
### 14:03
<!-- session:88727096-9116-4892-85aa-2a196a90b788 turn:a47e8e23-8e71-4625-90e0-189f2cd56da4 transcript:C:\Users\guido\.claude\projects\D--IT-Projects-Termdroid\88727096-9116-4892-85aa-2a196a90b788.jsonl -->
- User solicitó que la aplicación funcione en la mayor cantidad de versiones de Android posibles y sea fácil de usar, como instalar Claude Code y tipear "claude" en terminal.

- Claude Code identificó que la restricción W^X varía según la versión de Android del dispositivo, no solo el targetSdk, y propuso un modelo de tres niveles de ejecución en lugar de dos.

- Claude Code propuso implementar un "capability probe" en el primer arranque que detecte dinámicamente qué backend de ejecución usar en cada dispositivo específico, mejorando la arquitectura.

- Claude Code actualizó 25 archivos del proyecto e incorporó dos documentos técnicos nuevos con los cambios al plan.

- Se redefinió minSdk de 34 a 26 para mayor cobertura, se priorizó "cero fricción" (sin permisos iniciales, primer frame es el chat) y se implementó SSO para paridad con CLI.

- Se planearon APKs separados por ABI (arm64-v8a principal, x86_64 para emulador/Chromebook/WSA, armeabi-v7a), detección de OEM para guía de batería por fabricante, y matriz de prueba en múltiples dispositivos.

- Claude Code identificó que "tipear claude y funcione" depende del nivel 2 de ejecución, que no está garantizado en todos los dispositivos; si falla, la app nativa funciona pero el CLI se deshabilita.

- Se estableció C-001 (bootstrap Gradle con minSdk 26, splits por ABI y módulo :probe) como próximo paso en D:\IT\Projects\Termdroid, precediendo a F-001 para resolver el gate técnico.


## Session 15:21

### 15:22
<!-- session:88727096-9116-4892-85aa-2a196a90b788 turn:4f463469-0080-4df4-bd1f-6ada6a837917 transcript:C:\Users\guido\.claude\projects\D--IT-Projects-Termdroid\88727096-9116-4892-85aa-2a196a90b788.jsonl -->
- User estableció una meta de verificación punta a punta: implementar el proyecto validando que cada slice mantenga la misión, visión y objetivo intactos.

- Claude instaló el toolchain completo de Android (SDK, NDK r27c, CMake, Gradle 9.7.1) en la máquina y configuró dos emuladores (Android 9 API 28 y Android 16 API 36).

- Construyó un proyecto Gradle multi-módulo con AGP 9.3.2, compileSdk 37, targetSdk 37, minSdk 26, splits por ABI (arm64-v8a, x86_64, universal) y soporte nativo con CMake.

- Verificó F-001 (el gate de ejecución) midiendo en hardware real (emuladores) con SELinux en Enforcing: nivel 0 (`execve` directo) falla en API 36 pero funciona en API 28; nivel 2 (linker explícito) funciona en API 36; el probe eligió backends correctos en 49-53 ms.

- Implementó F-003 (emulador VT): parser de escape sequences como máquina de estados, buffer con scrollback y soporte de color, ShellSession uniendo PTY y parser, renderer Compose sobre Canvas, 21 tests JVM en verde.

- Ejecutó la app en ambas versiones de Android y descubrió tres defectos reales al correrla: zombies porque `close()` no cosechaba el hijo, carrera de hilos entre lector de PTY y UI (vaciaba pantalla al abrir teclado), superposición de líneas al renderizar texto largo.

- Arregló los tres defectos con tests de regresión; suite completa pasando (32 tests), shell ejecutándose como hijo de la app sin zombies, comandos (`ls`, `uname`, `id`) rendereándose correctamente.

- Documentó que Android 9 trae 210 comandos de toybox integrados (`sh`, `awk`, `sed`, `grep`, `find`, `tar`, `vi`, `ps`, `top`), reordenando el roadmap: F-003/F-004 ya no dependen de F-002.

- Verificó que la app cumple con las invariantes de misión (targetSdk 37, soporta múltiples Androids, detecta en lugar de asumir, degrada sin romper, sin fricción al arrancar), pero dejó sin implementar F-004 (agente), F-005 (tools de Android) y F-002 (rootfs).

- Próximo paso indicado: conectar el A56 real del usuario por depuración inalámbrica para correr F-001 en arm64 y validar que el gate se sostiene en hardware Samsung antes de continuar con F-004.
