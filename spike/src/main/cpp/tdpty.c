...
Java_com_termdroid_spike_NativePty_open(JNIEnv *env, jclass clazz,
...
Java_com_termdroid_spike_NativePty_read(JNIEnv *env, jclass clazz, jint fd, jbyteArray buf) {
    (void) clazz;
...
JNIEXPORT jint JNICALL
Java_com_termdroid_spike_NativePty_write(JNIEnv *env, jclass clazz, jint fd, jbyteArray buf) {
    (void) clazz;
    jsize len = (*env)->GetArrayLength(env, buf);
    jbyte *b = (*env)->GetByteArrayElements(env, buf, NULL);
...
Java_com_termdroid_spike_NativePty_resize(JNIEnv *env, jclass clazz, jint fd, jint rows, jint cols) {
    (void) env;
    (void) clazz;
    struct winsize ws;
    memset(&ws, 0, sizeof(ws));
...
JNIEXPORT jint JNICALL
Java_com_termdroid_spike_NativePty_waitFor(JNIEnv *env, jclass clazz, jint pid) {
    (void) env;
    (void) clazz;
    int status = 0;
...
JNIEXPORT void JNICALL
Java_com_termdroid_spike_NativePty_close(JNIEnv *env, jclass clazz, jint fd, jint pid) {
    (void) env;
    (void) clazz;
    if (fd >= 0) close(fd);
